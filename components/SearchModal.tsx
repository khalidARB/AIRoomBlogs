...
import { Search, X, Calendar, Clock, ArrowUpRight, FileText } from 'lucide-react';
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden z-10 flex flex-col max-h-[80vh]"
          >
            {/* Search Header Bar */}
            <div className="p-4 sm:p-6 border-b border-gray-100 flex items-center gap-3">
              ...
              <input
                ...
                autoFocus
                className="w-full text-base sm:text-lg font-medium text-[#111827] placeholder-gray-400 bg-transparent focus:outline-none"
              />
              {query && (
                <button
                  onClick={() => setQuery('')}
              ...
              <button
                ...
                className="hidden sm:flex items-center gap-1 text-xs font-semibold text-gray-400 bg-gray-100 px-2.5 py-1 rounded-md border border-gray-200 hover:bg-gray-200 transition-colors cursor-pointer"
              >
                ESC
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto divide-y divide-gray-100 flex-grow">
              ...
              {filteredPosts.length > 0 ? (
                <div className="space-y-3 pt-2">
                  {filteredPosts.map((post) => (
                    <Link
                      ...
                      href={`/blog/${post.slug}`}
                      onClick={onClose}
                      className="group flex items-start gap-4 p-3 rounded-2xl hover:bg-gray-50 transition-all duration-200 border border-transparent hover:border-gray-200/80"
                    >
                      <img
                      ...
                      <div className="flex-grow min-w-0">
                        ...
                        <h4 className="font-bold text-[#111827] text-sm sm:text-base leading-snug group-hover:text-lime-600 transition-colors truncate">
                          {post.title}
                        </h4>
                ...
                <div className="py-12 text-center flex flex-col items-center justify-center">
                  ...
                  <h4 className="font-bold text-[#111827] text-base mb-1">No articles found</h4>
            ...
            <div className="p-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between text-xs text-gray-400 px-6">
              <span className="flex items-center gap-1.5">
                Press <kbd className="px-1.5 py-0.5 bg-white border border-gray-200 rounded font-mono text-[10px] text-gray-600 shadow-xs">ESC</kbd> to exit
              </span>
              <span className="flex items-center gap-1.5">
                Powered by <strong className="text-gray-700 font-semibold">AiRooms Search</strong>
              </span>
            </div>
          </motion.div>
        </div>
